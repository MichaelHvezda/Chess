...
namespace Chess
{
    public class CzechAI
    {
        ...
        public static move_t MiniMaxAB(CzechChessBoard board, Player turn)
        {
            ...
            Dictionary<position_t, List<position_t>> moves = CzechLegalMoveSet.getPlayerMoves(board, turn);
            ...
            Parallel.ForEach(moves, (movelist, state, index) =>
            {
                ...
                foreach (position_t move in movelist.Value)
                {
                    ...
                    CzechChessBoard b2 = CzechLegalMoveSet.move(board, new move_t(movelist.Key, move), turn);
        ...
        private static int mimaab(CzechChessBoard board, Player turn, int depth, int alpha, int beta)
        {
            ...
            {
                List<CzechChessBoard> boards = new List<CzechChessBoard>();
                var moves = CzechLegalMoveSet.getLegalPawnInsert(board, turn);
                ...
                foreach (position_t pos in board.Pieces[turn])
                {
                    ...
                    {
                        if (STOP) return -1; // interupts
                        CzechChessBoard b2 = CzechLegalMoveSet.move(board, new move_t(pos, move), turn);
                        boards.Add(b2);
                    }
                }
                ...
                {
                    foreach (CzechChessBoard b2 in boards)
                    {
