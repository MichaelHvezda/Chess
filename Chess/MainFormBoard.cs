...
namespace Chess
{
    public partial class MainForm : Form, UIBoard
    {
        ...
        private void BoardClick(object sender, EventArgs e)
        {
            ...
            if (czechChess != null && !m_checkmate)
            {
                // clear board
                for (int i = 0; i < 8; i++)
                    for (int j = 0; j < 8; j++)
                ...
                for (int i = 0; i < 8; i++)
                {
                    ...
                    if (k > -1)
                    {
                        ...
                        if ((!m_manualBoard || m_finalizedBoard) && !m_aigame)
                        {
                            List<position_t> moves = czechChess.Select(new position_t(k, i));
                            foreach (position_t move in moves)
                            {
                        ...
                        {
                            var moves = czechChess.InsertPawn(new position_t(k, i));
                            if (moves)
                            {
                                czechChess.Board.SetPiece(m_manualPiece, m_manualPlayer, k, i);
                                SetPiece(m_manualPiece, m_manualPlayer, k, i);
                            }
